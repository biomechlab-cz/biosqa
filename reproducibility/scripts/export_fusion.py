...
from biosqa.export.redistributable import (  # noqa: E402
    assert_redistributable, restrict_to_redistributable)
...
def main():
    ...
    Xtr, ytr, _, dtr = store.load_modality(mod, "train"); ytr = ytr.astype(np.int64)
    ...
    # LICENSING GATE (Plan 09 Phase E). app/LICENSE-MODELS names the exact cohorts each
    # shipped graph may be trained on. That held only because store_v3 happened to
    # contain three EDA cohorts; this script takes whatever the store has, so exporting
    # against a fuller corpus (store_v9 has five) would put WESAD -- "scientific,
    # non-commercial purposes" only -- inside an MIT-licensed weight file, silently.
    # Restricting is a licensing decision and it is enforced here, not remembered.
    if args.redistributable:
        keep_tr = restrict_to_redistributable(mod, dtr)
        keep_va = restrict_to_redistributable(mod, dva)
        keep_te = restrict_to_redistributable(mod, dte)
        dropped = sorted(set(np.asarray(dtr).astype(str)) - set(np.asarray(dtr).astype(str)[keep_tr]))
        if dropped:
            print(f"[license] excluding {len(dropped)} non-redistributable cohort(s) "
                  f"from the shipped {mod.upper()} model: {dropped}", flush=True)
        Xtr, ytr, dtr = Xtr[keep_tr], ytr[keep_tr], np.asarray(dtr)[keep_tr]
        Xva, yva, dva = Xva[keep_va], yva[keep_va], np.asarray(dva)[keep_va]
        Xte, yte, dte = Xte[keep_te], yte[keep_te], np.asarray(dte)[keep_te]
        for split_ds in (dtr, dva, dte):
            assert_redistributable(mod, split_ds)
        print(f"[license] {mod.upper()} trains on: {sorted(set(np.asarray(dtr).astype(str)))}",
              flush=True)
