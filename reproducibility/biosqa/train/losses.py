...
class BoundaryPairLoss(nn.Module):
    """Pairwise margin on ADJACENT grades only — the region where the model is at chance.

    Plan 08 §20 measured usable-AUROC of 0.717-0.785 on unambiguous Q0/Q3 windows and
    only 0.544-0.614 on the ambiguous Q1/Q2 pair, i.e. discrimination collapses exactly
    where the usable/unusable call is made. Ordinary CE and SORD spend most of their
    gradient separating Q0 from Q3, which the model already does well.

    This adds, as an auxiliary term, a ranking margin over in-batch pairs whose grades
    are ADJACENT (``|y_i - y_j| == 1``), scoring each window by its expected grade
    ``s = sum_q q * p_q``. With ``operational_only=True`` the pairs are further
    restricted to the Q1/Q2 crossing, which is the deployed decision boundary.

    Auxiliary by construction: it is added to a primary loss, never used alone, and it
    changes no head, decode or exported op. Returns 0 when a batch contains no eligible
    pair, so it degrades gracefully on homogeneous batches.
    """

    def __init__(self, n_classes: int = 4, margin: float = 0.5,
                 operational_only: bool = False, cut: int = 1):
        super().__init__()
        self.margin = float(margin)
        self.operational_only = bool(operational_only)
        self.cut = int(cut)          # the Q{cut} | Q{cut+1} crossing; 1 == Q1|Q2
        self.register_buffer("levels", torch.arange(n_classes, dtype=torch.float32))

    def forward(self, logits: torch.Tensor, target: torch.Tensor) -> torch.Tensor:
        p = torch.softmax(logits, dim=1)
        s = (p * self.levels.unsqueeze(0)).sum(1)               # [B] expected grade
        y = target.float()
        dy = y.unsqueeze(0) - y.unsqueeze(1)                    # [B, B]
        hi = dy == 1.0                                          # row j is one grade ABOVE row i
        if self.operational_only:
            lo_is_cut = (y.unsqueeze(1) == float(self.cut))     # the lower member is Q{cut}
            hi = hi & lo_is_cut
        if not bool(hi.any()):
            return logits.sum() * 0.0                           # keeps the graph, contributes nothing
        ds = s.unsqueeze(0) - s.unsqueeze(1)                    # s_j - s_i, matching dy
        viol = F.softplus(self.margin - ds)
        return (viol * hi.float()).sum() / hi.float().sum().clamp(min=1.0)


class WithinSubjectRankLoss(nn.Module):
    """Rank quality only WITHIN a subject, never across cohorts (Plan 10 B3).

    Cross-cohort grades are not on one scale: an ordinal latent score is identifiable
    only up to a monotone reparameterisation, so "Q2" in one cohort need not mean "Q2"
    in another. Comparing two windows from the SAME subject removes that problem by
    construction -- both share subject, device, session and (usually) annotator, so the
    cohort gauge cancels without needing an invariance penalty. This is why it is worth
    trying where MixStyle and GroupDRO failed: those attacked marginal feature shift,
    which instance normalisation had already removed.

    It also supplies the *overlap* that psychometric equating needs. IRT anchoring and
    Dawid-Skene both require items labelled in common across sources; the BioSQA
    cross-protocol anchor panel was prepared and archived unrated, so that overlap does
    not exist. Within-subject pairs provide it without any new annotation.

    For same-subject pairs with ``y_i > y_j`` the score ``s = sum_q q * p_q`` must satisfy
    ``s_i > s_j + margin``. Returns 0 when a batch holds no eligible pair, so it needs a
    subject-grouped sampler to be effective -- with random batches, same-subject pairs
    are far too rare.
    """
    ...
    def forward(self, logits: torch.Tensor, target: torch.Tensor,
        ...
        p = torch.softmax(logits, dim=1)
        s = (p * self.levels.unsqueeze(0)).sum(1)
        y = target.float()
        same = groups.unsqueeze(0) == groups.unsqueeze(1)
        higher = (y.unsqueeze(0) - y.unsqueeze(1)) > 0        # row j strictly above row i
        elig = same & higher
        if not bool(elig.any()):
            return logits.sum() * 0.0
        ds = s.unsqueeze(0) - s.unsqueeze(1)
        viol = F.softplus(self.margin - ds)
        return (viol * elig.float()).sum() / elig.float().sum().clamp(min=1.0)
