...
| **Feature attribution** | *Which property* of the signal? | PPG, EEG, EDA (fusion models) |
...
For the **fusion models** — PPG, EEG, and EDA, whose grade fuses a hand-crafted **SQI + dynamics vector** — the app computes an **exact group-Shapley attribution**. It ranks roughly **5–6 interpretable feature groups** by how much each pushes the grade toward *unusable* versus *usable*, relative to a clean-tier reference.
