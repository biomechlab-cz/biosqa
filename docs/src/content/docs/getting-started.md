...
No recording of your own is required. The repository ships a `dummy_data/` folder with short synthetic ECG/PPG/EEG/EDA records that exercise the whole pipeline. Start with the **ECG or EDA** record: those are the two modalities whose model weights ship with the app, so they grade end to end. The PPG and EEG records open, plot and export, but report that no model is bundled for them (see [Models](/biosqa/docs/models/)).
