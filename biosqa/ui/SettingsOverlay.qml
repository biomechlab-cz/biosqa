...
Item {
    id: root
    anchors.fill: parent
    visible: AppController.settingsOpen
    z: 300
    ...
    // dim + click-outside-to-close scrim
    Rectangle {
        anchors.fill: parent
        color: Qt.rgba(0, 0, 0, 0.5)
        MouseArea { anchors.fill: parent; onClicked: AppController.closeSettings() }
    }

    Rectangle {
        anchors.centerIn: parent
        width: Math.min(720, root.width - 80)          // wider panel, but never overflow a narrow window
        height: Math.min(col.implicitHeight + 40, root.height - 80)
        color: Theme.bgPanel
        ...
        ScrollView {
            ...
            ColumnLayout {
                ...
                Label {
                    ...
                    lineHeight: 1.3
                    color: (signalView.durationSec > 0 && guard.prefiltered) ? "#E0A32E" : Theme.textMuted
